...
const tarefasDisponiveis = db.tasks.filter(task => {
    ...
    // Utilizador ainda não fez esta tarefa
    if (!participante) {
        return true;
    }

    // Se já enviou prova ou já foi aprovada,
    // não mostrar novamente nas tarefas disponíveis
    if (
    ...
    // Se foi rejeitada, pode aparecer novamente
    return true;
});
